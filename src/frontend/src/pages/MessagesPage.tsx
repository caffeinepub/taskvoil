...
function formatDate(
  iso: string,
  lang: string,
  t: { messages: { justNow: string; minutesAgo: string; hoursAgo: string } },
): string {
  const d = new Date(iso);
  const now = new Date();
  ...
  if (diffMin < 60)
    return t.messages.minutesAgo.replace("{n}", String(diffMin));
  const diffH = Math.floor(diffMin / 60);
  if (diffH < 24) return t.messages.hoursAgo.replace("{n}", String(diffH));
...
export function MessagesPage() {
  ...
  function handleSend() {
    ...
    if (!sent) {
      toast.error(t.messages.contactInfoBlocked);
      return;
    }
    setNewMessage("");
  }
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-0.5">
                            ...
                            <span className="text-xs text-muted-foreground shrink-0 ml-2">
                              {formatDate(conv.lastMessageAt, lang, t)}
                            </span>
                          </div>
                          {conv.taskTitle && (
